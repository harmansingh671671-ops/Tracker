...
class OdysseyLiveWallpaperService : WallpaperService() {
    ...
    inner class OdysseyWallpaperEngine : Engine() {
        ...
        private fun renderWallpaper(canvas: Canvas) {
            ...
            // 3. TOP SAFE ZONE (Top ~36% is left 100% clean OLED pure dark for phone's clock & notifications)
            val topSafeZone = height * 0.36f

            // 4. 24-HOUR CADENCE SPECTRUM BAR (Starts right below phone clock & notifications)
            val specY = topSafeZone + height * 0.010f
            val specH = height * 0.035f
            ...
            // 5. ADAPTIVE SCHEDULE TIMELINE (5 Cards centered on currentHour, placed vertically center)
            val blockCount = 5
            val timelineStartY = specY + specH + height * 0.012f
            val cardHeight = height * 0.040f
            ...
            val headerY = timelineEndY + height * 0.015f
            val headerH = height * 0.065f
            ...
            canvas.drawText("ODYSSEY • CH. 0$chapter   DAY $activeDay OF 365", cardPad + 25f, headerY + headerH * 0.38f, textPaint)
            ...
            canvas.drawText("$rankBadge $rankName • Level $userLevel Cadence", cardPad + 25f, headerY + headerH * 0.78f, rankPaint)

            // Radial Planned Ring
            val ringRadius = headerH * 0.30f
            ...
            // 7. DAILY CADENCE DIRECTIVE & INTEGRITY CARD
            val dirY = headerY + headerH + height * 0.012f
