...
import { Moon, Clock, Calendar, Sparkles, X, ChevronRight } from "lucide-react";
...
export function EveningReminderModal() {
  ...
  const checkTomorrowSchedule = useCallback(async () => {
    ...
    // Trigger if after 20:00 (or if test query flag is set) and schedule is incomplete (< 16 hours planned)
  ...
  if (!isOpen && !isSchedulerOpen) return null;

  return (
    <>
      {/* 20:00 Evening Reminder In-App Pop-up */}
      {isOpen && (
      ...
      {/* Embedded Day Schedule Modal for Tomorrow */}
      {isSchedulerOpen && user && (
        <DayScheduleModal
          ...
          dayNumber={Math.max(1, (user.streak ?? 0) + 2)}
          userId={user.id}
          onSaved={checkTomorrowSchedule}
        />
      )}
    </>
  );
