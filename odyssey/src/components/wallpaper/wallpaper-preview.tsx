...
export const DEFAULT_CADENCE_BLOCKS: ScheduleBlock[] = [
  {
    id: "fb1",
    userId: "u",
    date: "today",
  ...
  },
  {
    ...
    startTime: "08:00",
    endTime: "09:30",
    title: "Kernel Refactor & Architecture",
    category: "work",
    status: "completed",
    tag: "Deep Focus",
    isCommitted: true,
  ...
  },
  {
    ...
    startTime: "09:30",
    endTime: "11:30",
    title: "Architecture Design & Core Flow",
    category: "work",
    status: "pending",
    tag: "Deep Focus",
    isCommitted: true,
...
export function WallpaperPreview({
  ...
  // Hours: Use user's real blocks if present, otherwise fall back to full 24h sample cadence
  const effectiveBlocks = useMemo(() => {
    if (data.blocks && data.blocks.length > 0) {
      return data.blocks;
    }
    return DEFAULT_CADENCE_BLOCKS;
  }, [data.blocks]);
  ...
  const effectivePlannedHours = useMemo(() => {
    ...
    return 20; // 20 hours planned in default cadence
  }, [data.blocks, data.plannedHours]);
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full pt-3 px-6 flex flex-col items-center justify-between z-10" style={{ minHeight: "175px" }}>
        ...
        {showClockGuide ? (
          ...
          <div className="my-auto flex flex-col items-center justify-center py-6 text-center">
            <span className="text-[10px] font-mono uppercase tracking-widest text-white/20 border border-white/10 px-3 py-1 rounded-full">
              Safe Space for Phone Clock
            </span>
          </div>
        )}
      </div>
      ...
      <div className="px-4.5 w-full z-10 mt-2">
        <div className="rounded-xl p-2 px-2.5 bg-[#13151D]/60 border border-white/10 backdrop-blur-md flex flex-col gap-1.5">
          ...
          <div className="relative w-full py-0.5">
            ...
            <div
              className="absolute top-1/2 -translate-y-1/2 -ml-1 pointer-events-none flex items-center justify-center transition-all duration-500"
              style={{ left: `${Math.min(98, Math.max(2, (currentHourFloat / 24) * 100))}%` }}
            >
      ...
      {/* 4. CORE SCHEDULE TIMELINE CARDS */}
      <div className="px-4.5 py-1.5 flex flex-col gap-1.5 z-10">
        ...
        {effectiveBlocks.slice(0, 7).map((block, idx) => {
          const sH = parseInt(block.startTime.split(":")[0], 10);
          let eH = parseInt(block.endTime.split(":")[0], 10);
          ...
          return (
            <div
              ...
              className={`rounded-xl p-2 pl-2.5 transition-all flex items-center justify-between border ${
                isActive
                  ? "bg-[#161924] border-amber-400/60 shadow-[0_0_18px_rgba(245,158,11,0.2)] relative overflow-hidden"
                  : isPast
                  ? "bg-[#13151D]/50 border-white/5 opacity-75"
                  : "bg-[#13151D]/70 border-white/10"
              }`}
            >
              {isActive && (
                <div className="absolute left-0 top-0 bottom-0 w-1 bg-amber-400 shadow-[0_0_8px_#F59E0B]" />
              )}
              ...
              <div className="text-[11px] font-semibold text-slate-200 truncate pl-2 flex items-center gap-1">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <span className="text-sm">{getHobbyIcon(h.icon)}</span>
